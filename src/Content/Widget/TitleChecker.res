...
let rec queryDomHelp = async (maybeUploadDialog, selector, n): promise<Dom.element> => {
  if n < 0 {
    Js.Promise2.reject(TestError("Not Found"))
  } else {
    let wait = await pause()
    let maybeEl: option<Dom.element> =
      maybeUploadDialog->Option.mapWithDefault(Document.querySelector(document, selector), dialog =>
        dialog->Element.querySelector(selector)
      )
    switch maybeEl {
    | None => await queryDomHelp(maybeUploadDialog, selector, n - 1)
